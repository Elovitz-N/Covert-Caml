...
(** type [user] is the type that stores the information of a user. *)

type t = { mutable users : user list }
(** type [t] is the type that stores the list of users. *)

val trim_quotes : string -> string
(** [trim_quotes str] returns the input string with all quotations
    removed. *)

val msgs_of_json : Yojson.Basic.t -> msg
(** [msgs_of_json json] returns a msg extracted from a json. *)

val users_of_json : Yojson.Basic.t -> user
(** [users_of_json json] returns a user extracted from a json. *)

val get_users : password -> user list
(** [get_users db] retrieves user information from a database file and
    returns a list of the users. *)

val check_user : password -> password -> bool
(** [check_user db] returns true if the given username exists in the
    database file. *)

val from_json : password -> t
(** [from_json db] retrieves information from a database file and
    returns that information in a type t. *)

val get_uname : session_id -> string -> string
(** [get_uname id db] returns the username associated with the session
    id [id]. Raises "DNE" if that session id does not exist. *)

val get_password : session_id -> string -> password
(** [get_password id db] returns the password associated with the
    session id [id]. Raises "DNE" if that session id does not exist.*)

val get_new_msgs : session_id -> string -> string
(** [get_new_msgs id db] returns a string of the new messages associated
...
(** [update_list key v check_k check_v json_lst delete_msgs] returns the
    ...
    checking field [check_k] against value [check_v]. It then updates
    the ([key],[v]) field for that user by calling [update_k]. If [key]
    = "new messages" and [delete_msgs] = true, then all new messages
    corresponding to the user corresponding to ([check_k],[check_v])
    will be deleted. Raises "unknown json type" if the json_lst field is
    not a json String or json List. *)
...
(** [update_json obj key v check_k check_v delete_msgs add] returns the
    updated json association list by calling
    [update_list key v check_k
     check_v x delete_msgs]. If [add] =
    true, then the json obj [obj] will be added to the user list. Raises
    "invalid user list" if [obj] is not a json association list. *)
...
(** [put_uname id new_uname] replaces the username in the database file
    ...
    id [id] and returns the unit value. *)
...
(** [put_passwd id new_passwd] replaces the password in the database
    ...
    session id [id] and returns the unit value. *)
...
(** [put_msg reciever new_msg] adds the msg [new_msg] to the new
    ...
    file and returns the unit value. *)

val put_id : string -> string -> unit
(** [put_id uname] replaces the session id in the database file with the
    specified session id [id] associated with the username [uname] and
    returns the unit value. Raises "DNE" if that username does not
    exist. *)
...
(** [add_user id uname passwd] adds a user to the database file with
    ...
    new messages array, and returns the unit value. This function should
    be called when a client creates an account with the service. *)

val delete_msgs : string -> unit
(** [delete_msgs id] deletes the new messages associated with the user
    that has session id [id] from the database file and returns the unit
    value. *)

val list_unames : string -> password
(*** [list_unames string] creates a list of the usernames from the
  database and returns the list in a string format. *)

val create_db : unit -> unit
(*** [create_db ()] creates the json database file and initializes an
  empty users array. Returns unit. *)
